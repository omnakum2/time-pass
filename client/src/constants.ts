...
//   2. Storage keys — localStorage + sessionStorage key names.
//   3. WebSocket / networking — default port + reconnect backoff.
...
export const STORAGE_KEYS = {
  ...
  pendingRoomId: 'pendingRoomId', // sessionStorage: room id carried across an invite redirect
  pendingHost: 'pendingHost',     // sessionStorage: host name carried across an invite redirect
} as const;

// ─── 3. WebSocket / networking ───────────────────────────────────────────────
export const WS_DEFAULT_PORT = '3001';   // fallback WS port when VITE_WS_PORT is unset
